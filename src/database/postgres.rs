...
use super::Database;
use crate::{poll::{self, *}, error::{ErrorKind, InternalError}};
...
impl Database<&mut postgres::Client> for PostgresDatabase {
    fn get_poll_by_id(conn: &mut postgres::Client, id: String) -> Result<Option<Poll>, ErrorKind> {
        let poll_row = match conn.query("SELECT * FROM polls WHERE id = $1 LIMIT 1", &[&id])?.pop() {
            Some(row) => row,
        ...
        let method = match method_discrim {
            ...
            _ => return Err(ErrorKind::Internal(InternalError::UnknownVotingMethodDiscriminant(method_discrim))),
        };
        ...
        for row in winners_rows {
            ...
            winners_vec.push(RankedCandidate {
                ...
                rank: match rank.try_into() {
                    Ok(rank) => rank,
                    Err(err) => return Err(ErrorKind::Internal(InternalError::InvalidWinnerRank(rank, err))),
                },
            });
        }
        ...
        let num_winners = match num_winners.try_into() {
            ...
            Err(e) => return Err(ErrorKind::Internal(InternalError::InvalidNumWinners(num_winners, e))),
        };
